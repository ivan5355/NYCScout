...
async function formatRecommendations(items, intent) {
    ...
    const itemDescriptions = items
        .map((item, i) => {
            if (intent.type === "restaurant" || item.location?.address) {
                return `${i + 1}. Name: ${item.name} | Cuisine: ${item.cuisine || "N/A"} | Borough: ${item.location?.borough || "N/A"} | Neighborhood: ${item.location?.neighborhood || "N/A"} | Address: ${item.location?.address || "N/A"} | Price: ${item.metrics?.price_range || "N/A"}`;
            } else {
