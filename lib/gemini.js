...
3. When clarifying, guide the user toward these filters naturally:
   ...
   - "Casual spot or something a little nicer?"
...
4. CONVERSATION CONTEXT — THIS IS CRITICAL:
   ...
   - For restaurants: once you have at least food type + location, you can recommend (budget and vibe are optional bonuses).
    ...
    if (priorContext.length > 0) {
        const summaries = priorContext.map((c) => {
            ...
            const filters = [
                ...
                intent.vibeSignal ? `vibe=${intent.vibeSignal}` : null,
