...
async function generatePatternAwareGreeting(userProfile) {
    ...
    if (userProfile.borough_tendency) {
        const tendencies = userProfile.borough_tendency instanceof Map
            ? userProfile.borough_tendency.entries()
            : Object.entries(userProfile.borough_tendency);

        for (const [borough, count] of tendencies) {
            if (count > topCount) {
                topBorough = borough;
                topCount = count;
            }
    ...
    if (userProfile.cuisine_preferences) {
        const preferences = userProfile.cuisine_preferences instanceof Map
            ? userProfile.cuisine_preferences.entries()
            : Object.entries(userProfile.cuisine_preferences);

        for (const [cuisine, count] of preferences) {
